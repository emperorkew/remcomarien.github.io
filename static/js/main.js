...
(function () {
  ...
  if (typed) {
    const words = ['molecules', 'reactions', 'data streams', 'processes', 'equations', 'problems'];
  ...
  if (threeContainer && typeof THREE !== 'undefined') {
    ...
    function getColors() {
      return isDark()
        ? { chem: 0x818cf8, cs: 0x22d3ee, bond: 0x334155 }
        : { chem: 0x6366f1, cs: 0x06b6d4, bond: 0xcbd5e1 };
    }
  ...
  if (tempSlider && concSlider && eaSlider && reactionCanvas) {
    ...
    function updateDemo() {
      ...
      document.getElementById('conc-value').textContent = C.toFixed(1);
      document.getElementById('ea-value').textContent = (Ea / 1000).toFixed(0);
    ...
    function drawGraph(currentT, C, Ea) {
      ...
      const lineColor = isDarkMode ? '#818cf8' : '#6366f1';
      const dotColor = isDarkMode ? '#22d3ee' : '#06b6d4';
      ...
      gradient.addColorStop(0, isDarkMode ? 'rgba(129,140,248,0.15)' : 'rgba(99,102,241,0.1)');
      ...
      ctx.fillStyle = isDarkMode ? 'rgba(34,211,238,0.15)' : 'rgba(6,182,212,0.12)';
      ctx.fill();
